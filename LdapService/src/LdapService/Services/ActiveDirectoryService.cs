...
namespace LdapService.Services
{
    public class ActiveDirectoryService : ILdapService
    {
        ...
        public ActiveDirectoryService(LdapSetting _ldapSetting, string _organizationalUnit)
        {
        ...
        public LdapEntry GetLdapEntry(string accountName)
        {
            ...
            {
                LdapSearchResults lsc = ldapConn.Search(BuildDn(organizationalUnit), LdapConnection.SCOPE_SUB, "sAMAccountName=" + accountName, null, false);
        ...
        public bool ValidateUser(string accountName, string password)
        {
            ...
            if (user == null)
                throw new CredentialNotFoundException(accountName);

            try
            {
                var ldapConnection = new LdapConnection();
                ldapConnection.Connect(ldapSetting.ServerAddress, 389);
                ldapConnection.Bind(BuildDn(user.DisplayName, organizationalUnit), password);
        ...
        private string BuildDn(string baseDn)
        {
            return string.Format("{0},{1}", baseDn, ldapSetting.BaseDn);
        }
    }
}
